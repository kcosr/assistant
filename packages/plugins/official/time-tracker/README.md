...
- Export generates an XLSX report from the current view and downloads it directly (no intermediate file vault). Each task row's Description cell includes the trimmed task description first when present, followed by unique entry notes as bullets.
