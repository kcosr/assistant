...
describe('PiSessionWriter', () => {
  it.each(['medium', 'xhigh', 'none', 'max'])(
    ...
    async (thinkingLevel) => {
      ...
      expect(thinkingEntry?.['thinkingLevel']).toBe(
        thinkingLevel === 'none' ? 'off' : thinkingLevel,
      );
        ...
        const selectedThinking = resolveSessionThinkingForRun({
          agent: {
            ...
            chat: {
              provider: 'pi',
              models: ['local/plain'],
              modelSettings: [{ id: 'local/plain' }],
            },
          },
          summary: { ...summary, thinking: 'xhigh' },
        });
        expect(selectedThinking).toBeUndefined();
        await writer.sync({
          summary,
          messages: [...messages, { role: 'user', content: 'Continue without thinking' }],
          modelSpec: 'local/plain',
          thinkingLevel: selectedThinking ?? 'off',
        });
        const switchedEntries = parseJsonLines(await fs.readFile(filePath, 'utf8'));
        expect(
          switchedEntries
  ...
  it('removes orphan tail entries when trimming after an explicit request', async () => {
    const baseDir = await createTempDir('pi-session-writer-trim-after-orphan-tail');
    const now = () => new Date('2026-02-01T00:00:00.000Z');
    const writer = new PiSessionWriter({ baseDir, now });

    const summary: SessionSummary = {
      sessionId: 'session-trim-after-orphan-tail',
      agentId: 'pi',
      createdAt: now().toISOString(),
      updatedAt: now().toISOString(),
      attributes: {
