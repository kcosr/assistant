...
export function buildSystemPrompt(
  ...
  let selectedInstructionSkillNames: string[] | undefined;
  ...
  if (optionsOrRegistry instanceof AgentRegistry) {
    ...
    selectedInstructionSkillNames = optionsOrRegistry.selectedInstructionSkillNames;
  ...
  if (hasToolContext) {
    const messageContextLines: string[] = [
      ...
      '- window-id: The active app window. Use this in panels_* calls when multiple windows are active.',
      '- pane-id: The active pane that owns the selected panel/tab.',
      '- pane-tab-count, pane-tab-panel-ids: The current tab group for the active pane.',
      ...
      'If no context line is available (headless agents), call panels_windows, panels_list, panels_selected, or panels_tree (includeContext true) to find the target panel, pane, window, and context.',
      'If multiple windows are active, include windowId in panels_* calls.',
      'Use pane-id for tab/split layout operations when possible. Use panel-id for panel-specific tools when required.',
