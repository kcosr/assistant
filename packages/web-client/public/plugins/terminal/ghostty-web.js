...
class q {
  ...
  static async loadFromPath(A) {
    ...
    const g = await WebAssembly.compile(B),
      E = await WebAssembly.instantiate(g, {
        env: {
          log: (C, I) => {
...
class V {
  constructor(A) {
    ...
    const B = this.exports.ghostty_wasm_alloc_opaque(),
      g = this.exports.ghostty_key_encoder_new(0, B);
  ...
  setOption(A, B) {
    ...
    (new DataView(this.exports.memory.buffer).setUint8(g, typeof B == 'boolean' ? (B ? 1 : 0) : B),
  ...
  encode(A) {
    const B = this.exports.ghostty_wasm_alloc_opaque(),
      g = this.exports.ghostty_key_event_new(0, B);
    ...
    if (
      ...
      const M = new TextEncoder().encode(A.utf8),
        a = this.exports.ghostty_wasm_alloc_u8_array(M.length);
      (new Uint8Array(this.exports.memory.buffer).set(M, a),
        this.exports.ghostty_key_event_set_utf8(C, a, M.length),
        this.exports.ghostty_wasm_free_u8_array(a, M.length));
    }
    const I = 32,
      D = this.exports.ghostty_wasm_alloc_u8_array(I),
      i = this.exports.ghostty_wasm_alloc_usize(),
      w = this.exports.ghostty_key_encoder_encode(this.encoder, C, D, I, i);
    if (w !== 0)
      throw (
        this.exports.ghostty_wasm_free_u8_array(D, I),
        this.exports.ghostty_wasm_free_usize(i),
        this.exports.ghostty_key_event_free(C),
        new Error(`Failed to encode key: ${w}`)
      );
    const s = E.getUint32(i, !0),
      N = new Uint8Array(this.exports.memory.buffer, D, s).slice();
    return (
      this.exports.ghostty_wasm_free_u8_array(D, I),
      this.exports.ghostty_wasm_free_usize(i),
      this.exports.ghostty_key_event_free(C),
...
const z = class K {
  constructor(A, B, g = 80, E = 24, C) {
    ...
    if (
      ...
      try {
        ...
        let w = D;
        (i.setUint32(w, C.scrollbackLimit ?? 1e4, !0),
          (w += 4),
          i.setUint32(w, C.fgColor ?? 0, !0),
          (w += 4),
          i.setUint32(w, C.bgColor ?? 0, !0),
          (w += 4),
          i.setUint32(w, C.cursorColor ?? 0, !0),
          (w += 4));
        for (let s = 0; s < 16; s++)
  ...
  write(A) {
    const B = typeof A == 'string' ? new TextEncoder().encode(A) : A,
      g = this.exports.ghostty_wasm_alloc_u8_array(B.length);
  ...
  getColors() {
    const A = this.exports.ghostty_render_state_get_bg_color(this.handle),
      B = this.exports.ghostty_render_state_get_fg_color(this.handle);
    return {
      background: {
        r: (A >> 16) & 255,
        g: (A >> 8) & 255,
  ...
  getViewport() {
    ...
    return (
      ...
      this.exports.ghostty_render_state_get_viewport(this.handle, this.viewportBufferPtr, A) < 0
        ? this.cellPool
  ...
  getScrollbackLine(A) {
    ...
    if (g < 0) return null;
    const E = [],
      C = this.memory.buffer,
      I = new Uint8Array(C, this.viewportBufferPtr, g * K.CELL_SIZE),
      D = new DataView(C, this.viewportBufferPtr, g * K.CELL_SIZE);
    for (let i = 0; i < g; i++) {
  ...
  readResponse() {
    ...
    const A = 256,
      B = this.exports.ghostty_wasm_alloc_u8_array(A);
    try {
  ...
  parseCellsIntoPool(A, B) {
    const g = this.memory.buffer,
      E = new Uint8Array(g, A, B * K.CELL_SIZE),
      C = new DataView(g, A, B * K.CELL_SIZE);
    for (let I = 0; I < B; I++) {
      const D = I * K.CELL_SIZE,
        i = this.cellPool[I];
...
class Z {
  ...
  get normal() {
    return (
      this._normalBuffer || (this._normalBuffer = new m(this.terminal, 'normal')),
      this._normalBuffer
    );
  }
  get alternate() {
    return (
      this._alternateBuffer || (this._alternateBuffer = new m(this.terminal, 'alternate')),
...
class j {
  ...
  translateToString(A = !1, B = 0, g = this._length) {
    const E = Math.max(0, Math.min(B, this._length)),
      C = Math.max(E, Math.min(g, this._length));
    let I = '';
    for (let D = E; D < C; D++) {
...
class P {
  ...
  attach() {
    (typeof this.container.hasAttribute == 'function' &&
      ...
      (this.container.hasAttribute('tabindex') || this.container.setAttribute('tabindex', '0'),
  ...
  handleKeyDown(A) {
    ...
    if (
      (this.onKeyCallback && this.onKeyCallback({ key: A.key, domEvent: A }),
    ...
    try {
      ...
      const C = A.key.length === 1 && A.key.charCodeAt(0) < 128 ? A.key.toLowerCase() : void 0,
        I = this.encoder.encode({
          action: E,
          key: B,
          mods: g,
          utf8: C,
...
class v {
  ...
  constructor(A) {
    ((this.terminal = A),
      ...
      (this.scannedRows = /* @__PURE__ */ new Set()));
  }
  /**
  ...
  cacheLink(A) {
    const { start: B } = A.range,
      g = this.terminal.buffer.active.getLine(B.y);
    if (g) {
      const D = g.getCell(B.x);
      if (!D) {
        const { start: w, end: s } = A.range,
          N = `r${w.y}:${w.x}-${s.x}`;
...
class X {
  ...
  provideLinks(A, B) {
    const g = [],
      E = /* @__PURE__ */ new Set(),
      C = this.terminal.buffer.active.getLine(A);
    if (!C) {
      B(void 0);
      return;
    }
...
const n = class r {
  ...
  provideLinks(A, B) {
    const g = [],
      E = this.terminal.buffer.active.getLine(A);
    if (!E) {
      B(void 0);
      return;
    }
    ...
    for (; I !== null; ) {
      ...
      (s.length < D.length && ((D = s), (w = i + D.length - 1)),
        D.length > 8 &&
          g.push({
            text: D,
            range: {
...
class $ {
  constructor(A, B = {}) {
    ...
    ((this.ctx = g),
      ...
      (this.theme = { ...f, ...B.theme }),
      (this.devicePixelRatio = B.devicePixelRatio ?? window.devicePixelRatio ?? 1),
      (this.palette = [
        this.theme.black,
        this.theme.red,
        this.theme.green,
  ...
  measureFont() {
    ...
    const g = B.measureText('M'),
      E = Math.ceil(g.width),
      C = g.actualBoundingBoxAscent || this.fontSize * 0.8,
      I = g.actualBoundingBoxDescent || this.fontSize * 0.2,
      D = Math.ceil(C + I) + 2,
      i = Math.ceil(C) + 1;
  ...
  render(A, B = !1, g = 0, E, C = 1) {
    ...
    const I = A.getCursor(),
      D = A.getDimensions(),
      i = E ? E.getScrollbackLength() : 0;
    ((U = A.needsFullRedraw) != null && U.call(A) && (B = !0),
    ...
    const N = this.selectionManager && this.selectionManager.hasSelection(),
      k = /* @__PURE__ */ new Set();
    if (
      ((this.currentSelectionCoords = N ? this.selectionManager.getSelectionCoords() : null),
    ...
    const M = /* @__PURE__ */ new Set(),
      a = this.hoveredHyperlinkId !== this.previousHoveredHyperlinkId,
      h = JSON.stringify(this.hoveredLinkRange) !== JSON.stringify(this.previousHoveredLinkRange);
    if (a) {
      for (let t = 0; t < D.rows; t++) {
        ...
        if (c) {
          for (const F of c)
            if (
              F.hyperlink_id === this.hoveredHyperlinkId ||
              F.hyperlink_id === this.previousHoveredHyperlinkId
            ) {
              M.add(t);
              break;
            }
    ...
    if (h) {
      if (this.previousHoveredLinkRange)
        for (
          let t = this.previousHoveredLinkRange.startY;
          t <= this.previousHoveredLinkRange.endY;
          t++
        )
          M.add(t);
      if (this.hoveredLinkRange)
        for (let t = this.hoveredLinkRange.startY; t <= this.hoveredLinkRange.endY; t++) M.add(t);
      this.previousHoveredLinkRange = this.hoveredLinkRange;
    }
    const G = /* @__PURE__ */ new Set();
    for (let t = 0; t < D.rows; t++)
      (g > 0 ? !0 : B || A.isRowDirty(t) || k.has(t) || M.has(t)) &&
        (G.add(t), t > 0 && G.add(t - 1), t < D.rows - 1 && G.add(t + 1));
    for (let t = 0; t < D.rows; t++) {
      if (!G.has(t)) continue;
      let c = null;
      if (g > 0)
    ...
    (g === 0 && I.visible && this.cursorVisible && this.renderCursor(I.x, I.y),
      E && C > 0 && this.renderScrollbar(g, i, D.rows, C),
      (this.lastCursorPosition = { x: I.x, y: I.y }),
      A.clearDirty());
  }
  /**
  ...
  renderCellBackground(A, B, g) {
    const E = B * this.metrics.width,
      C = g * this.metrics.height,
      I = this.metrics.width * A.width;
    ...
    let i = A.bg_r,
      w = A.bg_g,
      s = A.bg_b;
    (A.flags & e.INVERSE && ((i = A.fg_r), (w = A.fg_g), (s = A.fg_b)),
      (i === 0 && w === 0 && s === 0) ||
        ((this.ctx.fillStyle = this.rgbToCSS(i, w, s)),
        this.ctx.fillRect(E, C, I, this.metrics.height)));
  }
  /**
  ...
  renderCellText(A, B, g) {
    ...
    let i = '';
    if (
      (A.flags & e.ITALIC && (i += 'italic '),
      A.flags & e.BOLD && (i += 'bold '),
      (this.ctx.font = `${i}${this.fontSize}px ${this.fontFamily}`),
      D)
    )
      ...
      let M = A.fg_r,
        a = A.fg_g,
        h = A.fg_b;
      (A.flags & e.INVERSE && ((M = A.bg_r), (a = A.bg_g), (h = A.bg_b)),
        (this.ctx.fillStyle = this.rgbToCSS(M, a, h)));
    }
    A.flags & e.FAINT && (this.ctx.globalAlpha = 0.5);
    const w = E,
      s = C + this.metrics.baseline;
    let N;
    if (
      (A.grapheme_len > 0 && (k = this.currentBuffer) != null && k.getGraphemeString
        ? (N = this.currentBuffer.getGraphemeString(g, B))
        : (N = String.fromCodePoint(A.codepoint || 32)),
      this.ctx.fillText(N, w, s),
      A.flags & e.FAINT && (this.ctx.globalAlpha = 1),
      A.flags & e.UNDERLINE)
    ) {
  ...
  stopCursorBlink() {
    (this.cursorBlinkInterval !== void 0 &&
      (clearInterval(this.cursorBlinkInterval), (this.cursorBlinkInterval = void 0)),
      (this.cursorVisible = !0));
  }
  // ==========================================================================
  // Public API
  ...
  setCursorBlink(A) {
    A && !this.cursorBlink
      ...
      : !A && this.cursorBlink && ((this.cursorBlink = !1), this.stopCursorBlink());
  }
  /**
  ...
  renderScrollbar(A, B, g, E = 1) {
    const C = this.ctx,
      I = this.canvas.height / this.devicePixelRatio,
      D = this.canvas.width / this.devicePixelRatio,
      i = 8,
      w = D - i - 4,
      s = 4,
      N = I - s * 2;
...
const L = class Y {
  ...
  constructor(A, B, g, E) {
    ((this.selectionStart = null),
      ...
      (this.dirtySelectionRows = /* @__PURE__ */ new Set()),
      (this.selectionChangedEmitter = new J()),
  ...
  getSelection() {
    ...
    let { col: A, absoluteRow: B } = this.selectionStart,
      { col: g, absoluteRow: E } = this.selectionEnd;
    ...
    for (let D = B; D <= E; D++) {
      ...
      for (let M = s; M <= N; M++) {
        ...
        if (a && a.codepoint !== 0) {
          let h;
          if (a.grapheme_len > 0)
            if (D < C) h = this.wasmTerm.getScrollbackGraphemeString(D, M);
            else {
              const G = D - C;
...
`));
  ...
  dispose() {
    (this.selectionChangedEmitter.dispose(),
      ...
      this.boundContextMenuHandler &&
        (this.renderer.getCanvas().removeEventListener('contextmenu', this.boundContextMenuHandler),
        (this.boundContextMenuHandler = null)),
  ...
  attachEventListeners() {
    ...
    (A.addEventListener('mousedown', (B) => {
      if (B.button === 0) {
        ...
        ((this.selectionStart = { col: g.col, absoluteRow: C }),
          (this.selectionEnd = { col: g.col, absoluteRow: C }),
          (this.isSelecting = !0));
      }
    }),
      A.addEventListener('mousemove', (B) => {
        if (this.isSelecting) {
          ...
          const g = this.pixelToCell(B.offsetX, B.offsetY),
            E = this.viewportRowToAbsolute(g.row);
      ...
      (this.boundDocumentMouseMoveHandler = (B) => {
        if (this.isSelecting) {
          const g = A.getBoundingClientRect(),
            E = Math.max(g.left, Math.min(B.clientX, g.right)),
            C = Math.max(g.top, Math.min(B.clientY, g.bottom)),
            I = E - g.left,
            D = C - g.top;
          if (
            (B.clientX < g.left ||
            ...
            const i = this.pixelToCell(I, D),
              w = this.viewportRowToAbsolute(i.row);
      ...
      (this.boundContextMenuHandler = (B) => {
        if (
          ...
          ((this.textarea.value = E),
            ...
            this.textarea.setSelectionRange(0, E.length));
          ...
          setTimeout(() => {
            const E = () => {
              ((this.textarea.style.pointerEvents = 'none'),
                ...
                this.textarea.removeEventListener('blur', E));
            };
            (document.addEventListener('click', E, { once: !0 }),
              document.addEventListener('contextmenu', E, { once: !0 }),
              this.textarea.addEventListener('blur', E, { once: !0 }));
          }, 10));
      }),
  ...
  pixelToCell(A, B) {
    const g = this.renderer.getMetrics(),
      E = Math.floor(A / g.width),
      C = Math.floor(B / g.height);
    return {
  ...
  normalizeSelection() {
    ...
    let C = this.absoluteRowToViewport(B),
      I = this.absoluteRowToViewport(E);
...
class IA {
  ...
  constructor(A = {}) {
    ((this.unicode = {
      ...
      (this.dataEmitter = new J()),
      (this.resizeEmitter = new J()),
      (this.bellEmitter = new J()),
      (this.selectionChangeEmitter = new J()),
      (this.keyEmitter = new J()),
      (this.titleChangeEmitter = new J()),
      (this.scrollEmitter = new J()),
      (this.renderEmitter = new J()),
      (this.cursorMoveEmitter = new J()),
      ...
      (this.animateScroll = () => {
        ...
        const g = this.options.smoothScrollDuration ?? 100,
          E = this.targetViewportY - this.viewportY;
        if (Math.abs(E) < 0.01) {
      ...
      (this.handleMouseLeave = () => {
        ...
        (this.renderer &&
          this.wasmTerm &&
          ((this.renderer.hoveredHyperlinkId || 0) > 0 && this.renderer.setHoveredHyperlinkId(0),
      ...
      (this.handleClick = async (g) => {
        ...
        const E = this.canvas.getBoundingClientRect(),
          C = Math.floor((g.clientX - E.left) / this.renderer.charWidth),
          D = Math.floor((g.clientY - E.top) / this.renderer.charHeight),
          i = this.wasmTerm.getScrollbackLength();
        let w;
      ...
      (this.handleWheel = (g) => {
        ...
        if (
          (g.preventDefault(),
          g.stopPropagation(),
          this.customWheelEventHandler && this.customWheelEventHandler(g))
        )
          return;
        if (((C = this.wasmTerm) == null ? void 0 : C.isAlternateScreen()) ?? !1) {
          const i = g.deltaY > 0 ? 'down' : 'up',
            w = Math.min(Math.abs(Math.round(g.deltaY / 33)), 5);
          for (let s = 0; s < w; s++)
          ...
          if (g.deltaMode === WheelEvent.DOM_DELTA_PIXEL) {
            const w =
              ((D = (I = this.renderer) == null ? void 0 : I.getMetrics()) == null
                ? void 0
                : D.height) ?? 20;
            i = g.deltaY / w;
          } else
      ...
      (this.handleMouseUp = () => {
        this.isDraggingScrollbar &&
          ...
          (this.scrollbarDragStart = null),
          this.canvas &&
            ((this.canvas.style.userSelect = ''), (this.canvas.style.webkitUserSelect = '')),
          this.scrollbarVisible && this.getScrollbackLength() > 0 && this.showScrollbar());
      }),
  ...
  handleOptionChange(A, B, g) {
    if (B !== g)
      switch (A) {
        ...
        case 'fontSize':
          this.renderer &&
            (this.renderer.setFontSize(this.options.fontSize), this.handleFontChange());
          break;
        case 'fontFamily':
          this.renderer &&
            (this.renderer.setFontFamily(this.options.fontFamily), this.handleFontChange());
          break;
        case 'cols':
        case 'rows':
  ...
  parseColorToHex(A) {
    ...
    if (B) {
      const g = Number.parseInt(B[1], 10),
        E = Number.parseInt(B[2], 10),
        C = Number.parseInt(B[3], 10);
      return (g << 16) | (E << 8) | C;
    }
    return 0;
  }
  ...
  open(A) {
    ...
    try {
      ...
      (this.canvas.addEventListener('mousedown', (E) => {
        ...
        (this.selectionManager = new AA(this, this.renderer, this.wasmTerm, this.textarea)),
        ...
        (this.linkDetector = new v(this)),
        this.linkDetector.registerProvider(new X(this)),
        this.linkDetector.registerProvider(new _(this)),
        ...
        A.addEventListener('wheel', this.handleWheel, { passive: !1, capture: !0 }),
...
`,
  ...
  input(A, B = !1) {
    (this.assertOpen(),
      !this.options.disableStdin && (B ? this.dataEmitter.fire(A) : this.write(A)));
  }
  /**
   * Resize terminal
   */
  ...
  attachCustomKeyEventHandler(A) {
    ((this.customKeyEventHandler = A),
      this.inputHandler && this.inputHandler.setCustomKeyEventHandler(A));
  }
  /**
  ...
  scrollLines(A) {
    ...
    const B = this.getScrollbackLength(),
      E = Math.max(0, Math.min(B, this.viewportY - A));
    E !== this.viewportY &&
      ((this.viewportY = E),
  ...
  scrollToTop() {
    ...
    A > 0 &&
      this.viewportY !== A &&
      ((this.viewportY = A), this.scrollEmitter.fire(this.viewportY), this.showScrollbar());
  }
  /**
  ...
  smoothScrollTo(A) {
    ...
    const B = this.getScrollbackLength(),
      E = Math.max(0, Math.min(B, A));
    if ((this.options.smoothScrollDuration ?? 100) === 0) {
      ((this.viewportY = E),
        (this.targetViewportY = E),
        this.scrollEmitter.fire(Math.floor(this.viewportY)),
        B > 0 && this.showScrollbar());
      return;
    }
  ...
  dispose() {
    if (!this.isDisposed) {
      ((this.isDisposed = !0),
        ...
        this.scrollAnimationFrame &&
          (cancelAnimationFrame(this.scrollAnimationFrame), (this.scrollAnimationFrame = void 0)),
        this.mouseMoveThrottleTimeout &&
          (clearTimeout(this.mouseMoveThrottleTimeout), (this.mouseMoveThrottleTimeout = void 0)),
   ...
   */
  cleanupComponents() {
    (this.selectionManager && (this.selectionManager.dispose(), (this.selectionManager = void 0)),
      this.inputHandler && (this.inputHandler.dispose(), (this.inputHandler = void 0)),
      this.renderer && (this.renderer.dispose(), (this.renderer = void 0)),
      this.canvas &&
        this.canvas.parentNode &&
        (this.canvas.parentNode.removeChild(this.canvas), (this.canvas = void 0)),
      this.textarea &&
        this.textarea.parentNode &&
        (this.textarea.parentNode.removeChild(this.textarea), (this.textarea = void 0)),
      this.element &&
        (this.element.removeEventListener('wheel', this.handleWheel),
        this.element.removeEventListener('mousedown', this.handleMouseDown, { capture: !0 }),
      ...
      this.scrollbarHideTimeout &&
        (window.clearTimeout(this.scrollbarHideTimeout), (this.scrollbarHideTimeout = void 0)),
      this.linkDetector && (this.linkDetector.dispose(), (this.linkDetector = void 0)),
  ...
  processMouseMove(A) {
    ...
    const B = this.canvas.getBoundingClientRect(),
      g = Math.floor((A.clientX - B.left) / this.renderer.charWidth),
      C = Math.floor((A.clientY - B.top) / this.renderer.charHeight);
    let I = 0,
      D = null;
    const i = this.getViewportY(),
      w = Math.max(0, Math.floor(i));
    if (w > 0) {
    ...
    this.linkDetector
      ...
      .then((h) => {
        var G, U, t, c;
        if (
          h !== this.currentHoveredLink &&
          ((U = (G = this.currentHoveredLink) == null ? void 0 : G.hover) == null || U.call(G, !1),
          (this.currentHoveredLink = h),
          (t = h == null ? void 0 : h.hover) == null || t.call(h, !0),
          this.element && (this.element.style.cursor = h ? 'pointer' : 'text'),
          this.renderer)
        )
          if (h) {
            const F = ((c = this.wasmTerm) == null ? void 0 : c.getScrollbackLength()) || 0,
              S = this.getViewportY(),
              x = Math.max(0, Math.floor(S)),
              p = h.range.start.y - F + x,
              T = h.range.end.y - F + x;
            p < this.rows && T >= 0
  ...
  processScrollbarDrag(A) {
    ...
    const g = this.canvas.getBoundingClientRect(),
      C = A.clientY - g.top - this.scrollbarDragStart,
      i = g.height - 4 * 2,
      w = this.rows,
      s = B + w,
      N = Math.max(20, (w / s) * i),
      k = -C / (i - N),
      M = Math.round(k * B),
      a = this.scrollbarDragStartViewportY + M;
  ...
  showScrollbar() {
    (this.scrollbarHideTimeout &&
      ...
      this.scrollbarVisible
        ...
        : ((this.scrollbarVisible = !0), (this.scrollbarOpacity = 0), this.fadeInScrollbar()),
      this.isDraggingScrollbar ||
  ...
  fadeInScrollbar() {
    const A = Date.now(),
      B = () => {
        const g = Date.now() - A,
          E = Math.min(g / this.SCROLLBAR_FADE_DURATION_MS, 1);
  ...
  fadeOutScrollbar() {
    const A = Date.now(),
      B = this.scrollbarOpacity,
      g = () => {
        const E = Date.now() - A,
          C = Math.min(E / this.SCROLLBAR_FADE_DURATION_MS, 1);
...
class DA {
  ...
  dispose() {
    (this._resizeObserver && (this._resizeObserver.disconnect(), (this._resizeObserver = void 0)),
      this._resizeDebounceTimer &&
        (clearTimeout(this._resizeDebounceTimer), (this._resizeDebounceTimer = void 0)),
  ...
  fit() {
    ...
    const B = this._terminal,
      ...
      E = B.rows;
    if (
      !((A.cols === this._lastCols && A.rows === this._lastRows) || (A.cols === g && A.rows === E))
    ) {
  ...
  proposeDimensions() {
    ...
    const C = window.getComputedStyle(E),
      I = Number.parseInt(C.getPropertyValue('padding-top')) || 0,
      D = Number.parseInt(C.getPropertyValue('padding-bottom')) || 0,
      i = Number.parseInt(C.getPropertyValue('padding-left')) || 0,
      w = Number.parseInt(C.getPropertyValue('padding-right')) || 0,
      s = E.clientWidth,
      N = E.clientHeight;
    if (s === 0 || N === 0) return;
    const k = s - i - w - gA,
      M = N - I - D,
      a = Math.max(QA, Math.floor(k / g.width)),
      h = Math.max(BA, Math.floor(M / g.height));
    ...
    (A = this._terminal) != null &&
      ...
      (this._resizeObserver ||
        ((this._resizeObserver = new ResizeObserver((B) => {
          this._isResizing ||
            !B[0] ||
            (this._resizeDebounceTimer && clearTimeout(this._resizeDebounceTimer),
