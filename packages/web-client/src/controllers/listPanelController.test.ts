...
describe('ListPanelController keyboard shortcuts', () => {
  ...
  it('applies global tag scope even without local filters', () => {
    const bodyEl = document.createElement('div');
    document.body.appendChild(bodyEl);
...
describe('ListPanelController add dialog', () => {
  ...
  it('includes global scope tags when adding items', () => {
    const bodyEl = document.createElement('div');
    document.body.appendChild(bodyEl);
    ...
    controller.render('list-1', { id: 'list-1', name: 'List', items: [] });
    controller.openAddItemDialog('list-1');

    const chips = Array.from(
      document.querySelectorAll<HTMLSpanElement>('.tag-chip'),
