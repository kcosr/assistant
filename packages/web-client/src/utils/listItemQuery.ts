...
export function evaluateAql(query: AqlQuery, item: ListPanelItem): boolean {
  if (!query.where) {
    return true;
  }
...
function evaluateExpr(expr: AqlExpr, item: ListPanelItem): boolean {
  if (expr.type === 'and') {
...
function evaluateClause(clause: AqlClause, item: ListPanelItem): boolean {
...
export function sortItemsByOrderBy(
  ...
  const compare = (a: ListPanelItem, b: ListPanelItem): number => {
