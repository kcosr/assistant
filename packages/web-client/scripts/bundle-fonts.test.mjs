...
import { bundledFontSubsets, bundleFonts } from './bundle-fonts.mjs';
...
test('bundles every declared font face and its license', async () => {
  ...
  for (const font of bundledFonts) {
    ...
    assert.match(notice, new RegExp(`- ${font.label}:`));
    await fs.access(path.join(outputDir, 'licenses', `${font.id}-OFL-1.1.txt`));
