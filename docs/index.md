# Documentation Index

This index maps the repository documentation into predictable categories so it is easy to find specs, design notes, and package guides.

## Table of Contents

- [Getting Started](#getting-started)
- [Specifications and References](#specifications-and-references)
- [Design Documents](#design-documents)
- [Package Documentation](#package-documentation)
- [Plugin Documentation](#plugin-documentation)
- [Process and Policies](#process-and-policies)
- [Documentation Review](#documentation-review)

## Getting Started

- [Project README](../README.md)

## Specifications and References

- [Config Reference](CONFIG.md)
- [UI Specification](UI_SPEC.md)
- [Plugin SDK](PLUGIN_SDK.md)
- [Plugin Migration](PLUGIN_MIGRATION.md)
- [Plugin Panel Migration](PLUGIN_PANEL_MIGRATION.md)
- [Tool Approvals](TOOL_APPROVALS.md)
- [Shared Collection Utils](SHARED_COLLECTION_UTILS.md)
- [Repo Documentation Guidelines](REPO_DOCUMENTATION_GUIDELINES.md)

## Design Documents

- [Agents](design/agents.md)
- [Calendar Plugin](design/calendar-plugin.md)
- [Chat Message Handling](design/chat-message-handling.md)
- [Chat Renderer UI Spec](design/chat-renderer-ui-spec.md)
- [Content Blocks](design/content-blocks.md)
- [External Agents](design/external-agents.md)
- [Issue 586: Chat Rendering and Transcript Replay](design/issue-586-chat-rendering-and-transcript-replay.md)
- [Panel Layout UI Spec](design/panel-layout-ui-spec.md)
- [Panel Plugins](design/panel-plugins.md)
- [Persistent Views](design/persistent-views.md)
- [Preferences](design/preferences.md)
- [Unified Chat Event Architecture](design/unified-chat-event-architecture.md)

## Package Documentation

- [Agent Server](../packages/agent-server/README.md)
- [Assistant CLI](../packages/assistant-cli/README.md)
- [Coding Executor](../packages/coding-executor/README.md)
- [Coding Sidecar](../packages/coding-sidecar/README.md)
- [Mobile Web](../packages/mobile-web/README.md)
- [Notify Proxy](../packages/notify-proxy/README.md)
- [Push CLI](../packages/push-cli/README.md)
- [Shared](../packages/shared/README.md)
- [Web Client](../packages/web-client/README.md)

## Plugin Documentation

- [Plugin SDK](PLUGIN_SDK.md)

### Core Plugins

- [Agents Plugin](../packages/plugins/core/agents/README.md)
- [Chat Plugin](../packages/plugins/core/chat/README.md)
- [Panels Plugin](../packages/plugins/core/panels/README.md)
- [Sessions Plugin](../packages/plugins/core/sessions/README.md)

### Official Plugins

- [Artifacts Plugin](../packages/plugins/official/artifacts/README.md)
- [Diff Plugin](../packages/plugins/official/diff/README.md)
- [Files Plugin](../packages/plugins/official/files/README.md)
- [Links Plugin](../packages/plugins/official/links/README.md)
- [Lists Plugin](../packages/plugins/official/lists/README.md)
- [Notes Plugin](../packages/plugins/official/notes/README.md)
- [Terminal Plugin](../packages/plugins/official/terminal/README.md)
- [Time Tracker Plugin](../packages/plugins/official/time-tracker/README.md)
- [Time Tracker Spec](../packages/plugins/official/time-tracker/SPEC.md)
- [URL Fetch Plugin](../packages/plugins/official/url-fetch/README.md)

### Example Plugins

- [Hello Plugin](../packages/plugins/examples/hello/README.md)
- [Session Info Plugin](../packages/plugins/examples/session-info/README.md)
- [WS Echo Plugin](../packages/plugins/examples/ws-echo/README.md)

## Process and Policies

- [Agent Instructions](../AGENTS.md)
- [Changelog](../CHANGELOG.md)

## Documentation Review

- [Documentation Review Log](DOCUMENTATION_REVIEW_LOG.md)
